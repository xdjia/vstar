...
for file in files:
    with open('result/num_states_' + file, 'rb') as f:
...
# Set grid and figure size for slide
...
for i, file in enumerate(files):
    ...
    # Set the title for each subplot, bigger font for slide
    axes[i].set_title(file, fontsize=18)
    axes[i].grid(True)
    # Force scientific notation at 1e3
...
# Common x-axis label, larger font for slides
fig.supxlabel('Number of Membership Queries', fontsize=18)
# Common y-axis label, larger font for slides
fig.supylabel('F1 Score', fontsize=18)
...
plt.savefig('combined_subplots_marp_slide.png',
            format='png', bbox_inches='tight', dpi=96)

# Optional: save as PDF as well if needed
plt.savefig('combined_subplots_marp_slide.pdf',
